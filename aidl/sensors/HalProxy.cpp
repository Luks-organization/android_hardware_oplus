...
HalProxy::HalProxy() {
  static const std::string kMultiHalConfigFiles[] = {
      "/vendor/etc/sensors/hals.conf", "/odm/etc/sensors/hals.conf"};
...
Return<void> HalProxy::getSensorsList_2_1(
...
Return<void> HalProxy::getSensorsList(
    ISensorsV2_0::getSensorsList_cb _hidl_cb) {
...
Return<Result> HalProxy::injectSensorData(const V1_0::Event& event) {
  ...
  if (mCurrentOperationMode == OperationMode::NORMAL &&
    ...
    ALOGE(
        "An event with type != ADDITIONAL_INFO passed to injectSensorData "
        "while operation"
        " mode was NORMAL.");
    result = Result::BAD_VALUE;
  }
...
Return<void> HalProxy::configDirectReport(
...
std::shared_ptr<ISubHalWrapperBase> HalProxy::getSubHalForSensorHandle(
